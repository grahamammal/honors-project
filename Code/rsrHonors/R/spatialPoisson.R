poi_gp_arrpfit <- function(O=obs,
  ...
  n <-length(O)
  rk = rank*mul
  AP=chol2inv(chol(crossprod(X,X)))%*%t(X)
  PPERP <- diag(n)- X%*%AP

  batchlength=adapt[["batchlength"]];  n.batch=adapt[["n.batch"]];  niter <- batchlength*n.batch
  cat("MCMC chain size:", batchlength,"\n")
  # define prior parameters
  beta.b <- priors[["beta.normal"]];  s2.a   <- priors[["s2.IG"]][1];  s2.b   <- priors[["s2.IG"]][2];  phi.a  <- priors[["phi.Unif"]][1];  phi.b  <- priors[["phi.Unif"]][2]
  ...
  beta.lf<-function(beta){
    ...
    lf <- sum(dpois(O,exp(z),log=T)) - crossprod(beta)/(p*beta.b)
    return(lf)
  }
  ...
  delta.lf<-function(delta){ # delta is rank-m
    ...
    lf <- sum(dpois(O,exp(z),log=T))- 1/(2*sParams[s2indx]) * foo2
  ...
  phi.lf <- function(phi){
    ...
    lr <- (
      sum(dpois(O,exp(z),log=T))
      -0.5*1/sParams[s2indx] * foo2 # likelihood
  ...
  return(list(run.time=runtime,p.params = samples_s ,  arrp.params=samples_arrp, model = "rrp/arrp",rank=rank, eta.params = samples_eta, w.params = samples_w, accepts = accept_s, acceptw = accept_w))
}
