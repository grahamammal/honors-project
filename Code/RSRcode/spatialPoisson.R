poi_gp_arrpfit <- function(O=obs,
                           ...
                           X, # covariate matrix
                           mul=2, # not sure what mul is yet, i guess we multiply the rank estimate by this?
                           covfn = covfn, # covariance function
                           adapt=adapt, # runs of mcmc and length of runs
                           nu=nu, # spatial smoothness parameter
                           core = core, # cores to run on
                           starting=starting, # starting guess for parameters, i believe mcmc starts here so as to not burnout
                           tuning=tuning, # must effect mcmc, not sure how
                           priors=priors, # list of our priors
                           rank=50){ # rank to reduce spatial matrix to
